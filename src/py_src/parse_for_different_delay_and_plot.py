...
#
    ...
    """
#    # *** F:DN for idct
    #float(.823)/float(.7369),
    speed_up__l = [float(.823)/float(.7535),
            float(.823)/float(.7597),
            float(.823)/float(.7651),
             float(.823)/float(.774)]
#    
...
#        print str(el) + " " + str(max_temperature__switching_temp_speed_up__curve[el])
    ...
    for temp__el in max_temp_key_list:
        ...
        generate_graph_2d_for_one_set_of_input(ax, fig, data_x,
                data_y,"Speedup",
                "Swithing Point"+" [$^\circ$C]", str(temp__el), 1, counter,
                n_colors, False)
        counter +=1
